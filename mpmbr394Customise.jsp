...
{
	history.go(-1);
	window.open("mpmbr394Validate.jsp",null, "height=300,width=320,status=no,toolbar=no,menubar=no,location=no,resizable=yes,scrollbars=yes");
	return false;
}
</script>
...
<%
	...
	for(int cnt=0;cnt<(rec-1);cnt++)
	{
		try
		{
			...
			pst=cn.prepareStatement("update mpmbr394 set [source]=?,[amount]=? where code=?");
			...
			if(affected>0)
			{
				pst=cn.prepareStatement("update mpmbr394Data set [amount]=? where code=?");
