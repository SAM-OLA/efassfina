...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("mpmbr318Adel.jsp");
	}
	try
	{
		connect c = new connect();
   		Connection cnn = c.connectMethod();
		PreparedStatement pss=cnn.prepareStatement("delete from mpmbr318A");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into mpmbr318A values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");		
