...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("wdfir200del.jsp");
	}
	...
	try
	{
		...
		PreparedStatement pss=cnn.prepareStatement("delete from wdfir200");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into wdfir200 values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");		
