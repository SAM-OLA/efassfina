...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("mpmbr328Adel.jsp");
	}
	try
	{
		connect c = new connect();
   		Connection cnn = c.connectMethod();
		PreparedStatement pss=cnn.prepareStatement("delete from mpmbr328A");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into mpmbr328A values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");		
