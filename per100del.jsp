...
<%
	try
	{
		...
		for(int i=1;i<no;i++)
		{
			...
			PreparedStatement pss=cn.prepareStatement("delete * from per100 where S_NO='"+value+"'");
			pss.executeUpdate();
		}
		...
		while(rs.next())
		{
			...
			ResultSet rs2=st2.executeQuery("select * from per100 ");
			while(rs2.next())
			{
				String sno2=rs2.getString(1);
				if(sno2.trim()!= "T_99999")
				{
					PreparedStatement pss2=cn.prepareStatement("update per100 set S_NO='"+ici+"' where S_NO='"+sno2+"'");
					pss2.executeUpdate();
					ici++;
				}	
			}
		...
		cn.close();
		response.sendRedirect("per100.jsp");
	}
	catch(Exception e)
	{
