...
<form name"frm" method="POST" action="qdfir531Customise.jsp">
...
<%
	try
	{
		pst=cn.prepareStatement("select * from qdfir531");
		rs=pst.executeQuery();
...
%>
			<tr bgcolor="blue">
				...
				<td><input type="text" name="itemdescription<%=cnt%>" size="50" value="<%=rs.getString(2)%>"></td>
				<td><input type="text" name="amount<%=cnt%>" size="20" value="<%=rs.getString(3)%>" readonly></td>
			</tr>
<%
		}
	}
	...
	{
		out.println("QDFIR 531 View Error: "+e.toString());
	}
		cn.close();
%>
  </table>
