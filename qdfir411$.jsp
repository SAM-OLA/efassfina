...
<%
	...
	try
	{
		...
		ResultSet rs=st.executeQuery("select * from qdfir411");
		while(rs.next())
		{
			cn++;
		}
...
<title>Cube CBN EFASS System - QDFIR 411</title>
...
<br>
    ...
    <tr>
        <form action="qdfir411$.jsp" method="post" name="form411">
 	...
 	<tr>
 	<form action="qdfir411Customise.jsp" method="post">
    ...
    <%
    	...
    	for(int i=1;i<cn;i++) 
    	{
			...
			ResultSet rs=st2.executeQuery("SELECT QDFIR411.S_NO, QDFIR411.INSTITUTIONCODE, Banks.BankName, QDFIR411.TYPEOFPLACEMENTS, QDFIR411.ACCTNo, QDFIR411.AMOUNT, QDFIR411.TENOR,QDFIR411.EFFECTIVEDATE, QDFIR411.MATURITYDATE, QDFIR411.INTERESTRATE, QDFIR411.UPFRONTINTERESTRECEIVED, QDFIR411.ACCRUEDINTRECEIVABLE, QDFIR411.TIMESROLLEDOVER, QDFIR411.REMARKS FROM QDFIR411 INNER JOIN Banks ON QDFIR411.INSTITUTIONCODE = Banks.BankCode where S_NO='"+i+"'");
    	...
    	for(int i=cn;i<ccn;i++) 
    	{
				...
				out.println("<td ><input type=\"checkbox\" name=\"chec" +i+ "\" onClick=\"if (this.checked) {this.form.chec"+i+".value='1'} else {this.form.chec"+i+".value='0'} del()\"</td>");
				out.println("<td ><input type=\"text\" readonly name=\"count" +i+ "\" value="+i+ "></td >");
				out.println("<td ><input type=\"text\" readonly name=\"f1" +i+ "\" size=\"62\"  value=\"0\"></td >");	
	    	...
	    	String ss1="", ss2="";
    		Statement st2=cnn.createStatement();
		ResultSet rs=st2.executeQuery("select * from qdfir411 where S_NO='T_99999'");
		while(rs.next())
		{
			ss1=String.valueOf(rs.getString(6));
			ss2=String.valueOf(rs.getString(12));
