...
<%
	...
	try
	{
		PreparedStatement pss = cnn.prepareStatement("delete * from QDFIR901Data");
            pss.executeUpdate();
            pss.close();
		Statement st=cnn.createStatement();
		ResultSet rs=st.executeQuery("select * from qdfir901");
		while(rs.next())
		{
			cn++;
			PreparedStatement pstInsert = cnn.prepareStatement("Insert into qdfir901Data Values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
<title>Cube CBN EFASS System - QDFIR 901</title>
...
<table border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse" width="1154">
  <tr>
    ...
    <td width="1063">
	<p align="center"><b><font color="#336699" size="2">QDFIR 901 - SCHEDULE OF OTHER LOANS AND ADVANCES GRANTED</font></b></p>     
    </td>
    <td width="18">&nbsp;</td>
  </tr>
</table>
...
<%
		...
		ResultSet rs3=st3.executeQuery("select value from qdfir300Data where code='10900'");
		rs3.next();
%>
Note: Expected Total: <%=rs3.getString(1)%> 
    ...
    <tr>
        <form action="qdfir901$.jsp" method="post" name="form901">
 	...
 	<tr>
 	<form action="qdfir901Customise.jsp" method="post">
	...
	<td align="center"><b><font color="#336699" size="2">UNPAID PRINCIPAL / INTEREST</font></b></td>
    ...
    <%
    	...
    	for(int i=1;i<cn;i++) 
    	{
			...
			ResultSet rs=st2.executeQuery("SELECT * FROM QDFIR901Data where S_NO='"+i+"'");
    		...
    		String ss1="";
    		Statement st2=cnn.createStatement();
		ResultSet rs=st2.executeQuery("select * from QDFIR901 where S_NO='T_99999'");
		while(rs.next())
		{
		...
		out.println("<td><input type=\"text\" name=\"total3\" size=\"62\"></td >");
