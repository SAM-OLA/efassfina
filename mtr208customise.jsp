...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("mtr208del.jsp");
	}
	...
	try
	{
		...
		PreparedStatement pss=cnn.prepareStatement("delete from mtr208");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into mtr208 values(?,?,?,?,?,?,?,?)");		
			...
			acost+=Double.parseDouble(f6);
			thr+=Double.parseDouble(f7);
			pss.executeUpdate();
		...
		pss.setString(7,String.valueOf(acost)); 
		pss.setString(8,String.valueOf(thr)); 	
		pss.executeUpdate();
		cnn.close();
		%>
		<SCRIPT Language = "JavaScript">
		    alert("Data Successful saved");
		    window.location = 'mtr208.jsp';
	      </SCRIPT>
		<%
