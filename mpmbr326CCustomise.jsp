...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("mpmbr326Cdel.jsp");
	}
	try
	{
		connect c = new connect();
   		Connection cnn = c.connectMethod();
		PreparedStatement pss=cnn.prepareStatement("delete from mpmbr326C");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into mpmbr326C values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");		
