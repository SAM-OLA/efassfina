...
<table border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse" width="1154">
  <tr>
    ...
    <td width="1063">
<p align="center"><b><font color="#336699" size="2">QDFIR 411 - SCHEDULE OF PLACEMENTS WITH BANKS IN NIGERIA</font></b></p>     
    </td>
    <td width="18">&nbsp;</td>
  </tr>
</table>
...
<form name"frm" method="POST" action="qdfir411Customise.jsp">
...
<%
	try
	{
		pst=cn.prepareStatement("select * from qdfir411");
		rs=pst.executeQuery();
...
%>
			<tr bgcolor="blue">
				...
				<td><input type="text" name="typeofplacements<%=cnt%>" size="10" value="<%=rs.getString(4)%>"></td>
				<td><input type="text" name="accountno<%=cnt%>" size="10" value="<%=rs.getString(5)%>"></td>
				<td><input type="text" name="amount<%=cnt%>" size="10" value="<%=rs.getString(6)%>"></td>
				<td><input type="text" name="tenor<%=cnt%>" size="10" value="<%=rs.getString(7)%>" readonly></td>
				<td><input type="text" name="effectivedate<%=cnt%>" size="10" value="<%=rs.getString(8)%>" readonly></td>
				<td><input type="text" name="maturitydate<%=cnt%>" size="10" value="<%=rs.getString(9)%>" readonly></td>
				<td><input type="text" name="interestrate<%=cnt%>" size="10" value="<%=rs.getString(10)%>" readonly></td>
				<td><input type="text" name="upfrontinterestreceived<%=cnt%>" size="10" value="<%=rs.getString(11)%>" readonly></td>
				<td><input type="text" name="accruesintreceivable<%=cnt%>" size="10" value="<%=rs.getString(12)%>" readonly></td>
				<td><input type="text" name="timesrolledover<%=cnt%>" size="10" value="<%=rs.getString(13)%>" readonly></td>
...
<%
	...
	{
		out.println("QDFIR 411 View Error: "+e.toString());
	}
		cn.close();
%>
  </table>
