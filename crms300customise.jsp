<%@ page import="java.sql.*, cbnbas.crms.connect" session="true"%>
 <%
	...
	if(flag==true)
	{
		response.sendRedirect("crms300del.jsp");
	}
	...
	try
	{
		...
		PreparedStatement pss=cnn.prepareStatement("delete from crms300");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			String f24=request.getParameter("bf24"+i);
			String f25=request.getParameter("bf25"+i);
			String f26=request.getParameter("bf26"+i);
			String f27=request.getParameter("bf27"+i);
			String f28=request.getParameter("bf28"+i);
			String f29=request.getParameter("bf29"+i);
			String f30=request.getParameter("bf30"+i);
			String f31=request.getParameter("cf31"+i);
			String f32=request.getParameter("cf32"+i);
			String f33=request.getParameter("cf33"+i);
			
			pss=cnn.prepareStatement("insert into crms300 values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");		
