...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("mpmbr326Adel.jsp");
	}
	try
	{
		connect c = new connect();
   		Connection cnn = c.connectMethod();
		PreparedStatement pss=cnn.prepareStatement("delete from mpmbr326A");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into mpmbr326A values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");		
