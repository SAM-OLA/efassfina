...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("mpmbr332Cdel.jsp");
	}
	try
	{
		connect c = new connect();
   		Connection cnn = c.connectMethod();
		PreparedStatement pss=cnn.prepareStatement("delete from mpmbr332C");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into mpmbr332C values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");		
