...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("qdfir541del.jsp");
	}
	try
	{
		connect c = new connect();
   		Connection cnn = c.connectMethod();
		PreparedStatement pss=cnn.prepareStatement("delete from qdfir541");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into qdfir541 values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");		
