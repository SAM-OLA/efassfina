...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("mpmbr412del.jsp");
	}
	try
	{
		connect c = new connect();
   		Connection cnn = c.connectMethod();
		PreparedStatement pss=cnn.prepareStatement("delete from mpmbr412");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into mpmbr412 values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");		
