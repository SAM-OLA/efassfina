...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("wdfir500del.jsp");
	}
	...
	try
	{
		...
		PreparedStatement pss=cnn.prepareStatement("delete from wdfir500");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into wdfir500 values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");		
