...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("sdfir1910del.jsp");
	}
	...
	try
	{
		...
		PreparedStatement pss=cnn.prepareStatement("delete from sdfir1910");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			String f27=request.getParameter("f27"+i);
			String f28=request.getParameter("f28"+i);
			String f29=request.getParameter("f29"+i);
			
			
			
			pss=cnn.prepareStatement("insert into sdfir1910 values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");		
