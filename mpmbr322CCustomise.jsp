...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("mpmbr322Cdel.jsp");
	}
	try
	{
		connect c = new connect();
   		Connection cnn = c.connectMethod();
		PreparedStatement pss=cnn.prepareStatement("delete from mpmbr322C");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into mpmbr322C values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");		
