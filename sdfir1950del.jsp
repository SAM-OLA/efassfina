...
<%
	try
	{
		...
		for(int i=1;i<no;i++)
		{
			...
			PreparedStatement pss=cn.prepareStatement("delete * from sdfir1950 where S_NO='"+value+"'");
			pss.executeUpdate();
		}
		...
		while(rs.next())
		{
			...
			ResultSet rs2=st2.executeQuery("select * from sdfir1950 ");
			while(rs2.next())
			{
				String sno2=rs2.getString(1);
				if(sno2.trim()!= "Total")
				{
					PreparedStatement pss2=cn.prepareStatement("update sdfir1950 set S_NO='"+ici+"' where S_NO='"+sno2+"'");
					pss2.executeUpdate();
					ici++;
				}	
			}
		...
		cn.close();
		response.sendRedirect("sdfir1950.jsp");
	}
	catch(Exception e)
	{
