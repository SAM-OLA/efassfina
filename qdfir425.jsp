...
<%
	...
	try
	{
		PreparedStatement pss = cnn.prepareStatement("delete * from QDFIR425Data");
            pss.executeUpdate();
            pss.close();
		...
		{
			cn++;
			PreparedStatement pstInsert = cnn.prepareStatement("Insert into qdfir425Data Values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
<br>
    ...
    <%
    	...
    	for(int i=1;i<cn;i++) 
    	{
			...
			ResultSet rs=st2.executeQuery("SELECT * FROM QDFIR425Data where S_NO='"+i+"'");
    		...
    		String ss1="";
    		Statement st2=cnn.createStatement();
		ResultSet rs=st2.executeQuery("select * from QDFIR425Data where S_NO='T_99999'");
		while(rs.next())
		{
