...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("wdfir300del.jsp");
	}
	...
	try
	{
		...
		PreparedStatement pss=cnn.prepareStatement("delete from wdfir300");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into wdfir300 values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");		
