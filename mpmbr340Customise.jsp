...
{
	history.go(-1);
	window.open("mpmbr340Validate.jsp",null, "height=300,width=320,status=no,toolbar=no,menubar=no,location=no,resizable=yes,scrollbars=yes");
	return false;
}
</script>
...
<%
	...
	{
		out.println("mpmbr340 Error: "+e.toString());
	}
	...
	for(int cnt=0;cnt<rowCnt;cnt++)
	{
		...
		t11=request.getParameter("t11"+cnt);
		...
		try
		{
			...
			pst=cn.prepareStatement("update MPMBR340 set [TERM LOAN]=?,[OVER DRAFT]=?,[OTHERS]=?,[ADVANCES UNDER LEASE]=?,[BANKERS ACCEPTANCES]=?,[COMMERCIAL PAPERS]=?,[BILLS DISCOUNTED]=?,[OTHERS 2]=?,[TOTAL]=? where code=?");
			pst.setString(1,t3);
			pst.setString(2,t4);
			pst.setString(3,t5);
			pst.setString(4,t6);
		...
		{
			out.println("mpmbr340 update error: "+e.toString());
		}
	}
	cn.close();
%>
<hr>
<a href="mpmbr340.jsp">Back</a> | <a href="" onclick="return openValidate()">Validate now!</a></body>