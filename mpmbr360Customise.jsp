...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("mpmbr360del.jsp");
	}
	try
	{
		connect c = new connect();
   		Connection cnn = c.connectMethod();
		PreparedStatement pss=cnn.prepareStatement("delete from mpmbr360");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into mpmbr360 values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");		
		...
		pss.setString(19,String.valueOf(ls_format.format(anom1)));
