...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("per304del.jsp");
	}
	...
	try
	{
		...
		PreparedStatement pss=cnn.prepareStatement("delete from per304");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into per304 values(?,?,?,?,?,?,?,?,?,?,?,?)");		
			...
			anom+=Integer.parseInt(f6);
			num+=Integer.parseInt(f7);
			anum+=Double.parseDouble(f11);
			pss.executeUpdate();
		...
		pss.setString(12,String.valueOf(anum));	
		pss.executeUpdate();
		cnn.close();
		%>
		<SCRIPT Language = "JavaScript">
		    alert("Data Successful saved");
		    window.location = 'per304.jsp';
	      </SCRIPT>
		<%
	}
	catch(Exception e)
