...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("spmbr1960del.jsp");
	}
	...
	try
	{
		...
		PreparedStatement pss=cnn.prepareStatement("delete from spmbr1960");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into spmbr1960 values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");		
