...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("mpmbr388del.jsp");
	}
	try
	{
		connect c = new connect();
   		Connection cnn = c.connectMethod();
		PreparedStatement pss=cnn.prepareStatement("delete from mpmbr388");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into mpmbr388 values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");		
