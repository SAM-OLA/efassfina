...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("qdfir1650del.jsp");
	}
	try
	{
		connect c = new connect();
   		Connection cnn = c.connectMethod();
		PreparedStatement pss=cnn.prepareStatement("delete from qdfir1650");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into qdfir1650 values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");		
