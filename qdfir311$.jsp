...
<br>
    ...
    <%
    	...
    	for(int i=1;i<cn;i++) 
    	{
			...
			ResultSet rs=st2.executeQuery("SELECT * FROM QDFIR311 where S_NO='"+i+"'");
    	...
    	for(int i=cn;i<ccn;i++) 
    	{
				...
				out.println("<td ><input type=\"checkbox\" name=\"chec" +i+ "\" onClick=\"if (this.checked) {this.form.chec"+i+".value='1'} else {this.form.chec"+i+".value='0'} del()\"</td>");
				out.println("<td ><input type=\"text\" readonly name=\"count" +i+ "\" value="+i+ "></td >");
				out.println("<td ><input type=\"text\" readonly name=\"f1" +i+ "\" size=\"61\"  value=\"0\"></td >");	
	    			out.println("<td ><select name=\"f2" +i+ "\">");
				out.println("<option value=''>------Select------</option>");
				while(rsBanks.next())
					{
	    	...
	    	String ss1="";
    		Statement st2=cnn.createStatement();
		ResultSet rs=st2.executeQuery("select * from qdfir311 where S_NO='T_99999'");
		while(rs.next())
		{
