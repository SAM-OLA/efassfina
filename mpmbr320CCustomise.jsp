...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("mpmbr320Cdel.jsp");
	}
	try
	{
		connect c = new connect();
   		Connection cnn = c.connectMethod();
		PreparedStatement pss=cnn.prepareStatement("delete from mpmbr320C");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into mpmbr320C values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");		
