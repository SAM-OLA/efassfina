...
<%
	...
	try
	{
		PreparedStatement pss = cnn.prepareStatement("delete * from QDFIR701Data");
            pss.executeUpdate();
            pss.close();
		Statement st=cnn.createStatement();
		ResultSet rs=st.executeQuery("select * from qdfir701");
		while(rs.next())
		{
			cn++;
			PreparedStatement pstInsert = cnn.prepareStatement("Insert into qdfir701Data Values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
<title>Cube CBN EFASS System - QDFIR 701</title>
...
<br>
    ...
    <tr>
        <form action="qdfir701$.jsp" method="post" name="form701">
 	...
 	<tr>
 	<form action="qdfir701Customise.jsp" method="post">
    ...
    <%
    	...
    	for(int i=1;i<cn;i++) 
    	{
			...
			ResultSet rs=st2.executeQuery("SELECT * FROM QDFIR701Data where S_NO='"+i+"'");
    		...
    		String ss1="";
    		Statement st2=cnn.createStatement();
		ResultSet rs=st2.executeQuery("select * from QDFIR701Data where S_NO='T_99999'");
		while(rs.next())
		{
