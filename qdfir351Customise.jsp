...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("qdfir351del.jsp");
	}
	try
	{
		connect c = new connect();
   		Connection cnn = c.connectMethod();
		PreparedStatement pss=cnn.prepareStatement("delete from qdfir351");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into qdfir351 values(?,?,?,?,?,?)");		
		...
		pss.setString(6,String.valueOf(request.getParameter("total5")));
		pss.executeUpdate();
		cnn.close();
		%>
		<SCRIPT Language = "JavaScript">
		    alert("Data Successful saved");
		    window.location = 'qdfir351.jsp';
	      </SCRIPT>
		<%
