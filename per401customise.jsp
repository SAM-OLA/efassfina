...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("per401del.jsp");
	}
	...
	try
	{
		...
		PreparedStatement pss=cnn.prepareStatement("delete from per401");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into per401 values(?,?,?,?,?,?,?,?,?,?)");		
			...
			pss.setString(9,f8);
			pss.setString(10,f9);
			anom1+=Integer.parseInt(f7);
			anom2+=Double.parseDouble(f9);
			pss.executeUpdate();
		...
		pss.setString(8,String.valueOf(anom1)); 	
		pss.setString(9,String.valueOf(request.getParameter("total9"))); 	
		pss.setString(10,String.valueOf(anom2)); 	
		pss.executeUpdate();
		cnn.close();
		%>
		<SCRIPT Language = "JavaScript">
		    alert("Data Successful saved");
		    window.location = 'per401.jsp';
	      </SCRIPT>
		<%
	}
	catch(Exception e)
