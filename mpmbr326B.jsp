...
<%
	...
	try
	{
		PreparedStatement pss = cnn.prepareStatement("delete * from MPMBR326BData");
            pss.executeUpdate();
            pss.close();
		...
		{
			cn++;
			PreparedStatement pstInsert = cnn.prepareStatement("Insert into MPMBR326BData Values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
<br>
    ...
    <%
    	...
    	for(int i=1;i<cn;i++) 
    	{
			...
			ResultSet rs=st2.executeQuery("SELECT * FROM MPMBR326BData where S_NO='"+i+"'");
    		...
    		String ss1="";
    		Statement st2=cnn.createStatement();
		ResultSet rs=st2.executeQuery("select * from MPMBR326BData where S_NO='T_99999'");
		while(rs.next())
		{
