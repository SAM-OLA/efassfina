...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("qdfir421del.jsp");
	}
	try
	{
		connect c = new connect();
   		Connection cnn = c.connectMethod();
		PreparedStatement pss=cnn.prepareStatement("delete from qdfir421");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into qdfir421 values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");		
