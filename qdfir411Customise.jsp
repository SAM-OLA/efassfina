...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("qdfir411del.jsp");
	}
	try
	{
		connect c = new connect();
   		Connection cnn = c.connectMethod();
		PreparedStatement pss=cnn.prepareStatement("delete from qdfir411");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into qdfir411 values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");		
			...
			anom2+=Long.parseLong(f5);
			anom3+=Long.parseLong(f11);
			pss.executeUpdate();
