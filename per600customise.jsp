...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("per600del.jsp");
	}
	...
	try
	{
		...
		PreparedStatement pss=cnn.prepareStatement("delete from per600");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into per600 values(?,?,?,?,?,?,?,?,?,?,?,?,?)");		
		...
		pss.setString(9,String.valueOf(anom1)); 	
