...
<%
	...
	try
	{
		PreparedStatement pss = cnn.prepareStatement("delete * from QDFIR461Data");
            pss.executeUpdate();
            pss.close();
		Statement st=cnn.createStatement();
		ResultSet rs=st.executeQuery("select * from qdfir461");
		while(rs.next())
		{
			cn++;
			PreparedStatement pstInsert = cnn.prepareStatement("Insert into qdfir461Data Values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
<title>Cube CBN EFASS System - QDFIR 461</title>
...
<%
		...
		ResultSet rs3=st3.executeQuery("select value from qdfir300Data where code='20460'");
		rs3.next();
%>
Note: Expected Total: <%=rs3.getString(1)%> 
    ...
    <tr>
        <form action="qdfir461$.jsp" method="post" name="form461">
 	...
 	<tr>
 	<form action="qdfir461Customise.jsp" method="post">
    ...
    <%
    	...
    	for(int i=1;i<cn;i++) 
    	{
			...
			ResultSet rs=st2.executeQuery("SELECT * FROM QDFIR461Data where S_NO='"+i+"'");
    		...
    		String ss1="";
    		Statement st2=cnn.createStatement();
		ResultSet rs=st2.executeQuery("select * from QDFIR461Data where S_NO='T_99999'");
		while(rs.next())
		{
