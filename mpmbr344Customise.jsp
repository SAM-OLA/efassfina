...
{
	history.go(-1);
	window.open("mpmbr344Validate.jsp",null, "height=300,width=320,status=no,toolbar=no,menubar=no,location=no,resizable=yes,scrollbars=yes");
	return false;
}
</script>
...
<%
	...
	{
		out.println("mpmbr344 Error: "+e.toString());
	}
	...
	for(int cnt=0;cnt<rowCnt;cnt++)
	{
		...
		try
		{
			...
			pst=cn.prepareStatement("update MPMBR344 set [1-30DaysN]=?,[31-60DaysN]=?,[61-90DaysN]=?,[91-180DaysN]=?,[181-360DaysN]=?,[Above360DaysN]=?,[TotalN]=?,[TotalPercent]=? where code=?");
			pst.setString(1,t3);
			pst.setString(2,t4);
			pst.setString(3,t5);
			pst.setString(4,t6);
		...
		{
			out.println("mpmbr344 update error: "+e.toString());
		}
	}
	cn.close();
%>
<hr>
<a href="mpmbr344.jsp">Back</a> | <a href="" onclick="return openValidate()">Validate now!</a></body>