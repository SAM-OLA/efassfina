...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("per201del.jsp");
	}
	...
	try
	{
		...
		PreparedStatement pss=cnn.prepareStatement("delete from per201");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into per201 values(?,?,?,?,?,?,?,?,?)");		
			...
			pss.setString(8,f7);
			pss.setString(9,f8);
			anom+=Integer.parseInt(f7);
			acost+=Integer.parseInt(f8);
			pss.executeUpdate();
		...
		pss.setString(8,String.valueOf(anom)); 
