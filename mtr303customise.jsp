...
<%
	...
	if(flag==true)
	{
		response.sendRedirect("mtr303del.jsp");
	}
	...
	try
	{
		...
		PreparedStatement pss=cnn.prepareStatement("delete from mtr303");
		pss.executeUpdate();
		for(int ii=1;ii<=cnt;ii++)
		{
			...
			pss=cnn.prepareStatement("insert into mtr303 values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");		
			...
			anom+=Double.parseDouble(f8);
			anum+=Integer.parseInt(f9);
			pss.executeUpdate();
		...
		pss.setString(9,String.valueOf(anom)); 	
		pss.setString(10,String.valueOf(anum)); 
