...
<form name"frm" method="POST" action="qdfir541Customise.jsp">
...
<%
	try
	{
		pst=cn.prepareStatement("select * from qdfir541");
		rs=pst.executeQuery();
...
%>
			<tr bgcolor="blue">
				...
				<td><input type="text" name="amountinvested<%=cnt%>" size="8" value="<%=rs.getString(3)%>"></td>
				<td><input type="text" name="invcertno<%=cnt%>" size="8" value="<%=rs.getString(4)%>"></td>
				<td><input type="text" name="tenor<%=cnt%>" size="8" value="<%=rs.getString(5)%>"></td>
				<td><input type="text" name="effectivedate<%=cnt%>" size="5" value="<%=rs.getString(6)%>"></td>
				<td><input type="text" name="maturitydate<%=cnt%>" size="5" value="<%=rs.getString(7)%>" readonly></td>
				<td><input type="text" name="interestrate<%=cnt%>" size="5" value="<%=rs.getString(8)%>" readonly></td>
				<td><input type="text" name="upfrontinterestpaid<%=cnt%>" size="5" value="<%=rs.getString(9)%>" readonly></td>
				<td><input type="text" name="accruedintpayable<%=cnt%>" size="5" value="<%=rs.getString(10)%>" readonly></td>
				<td><input type="text" name="timesrolledover<%=cnt%>" size="5" value="<%=rs.getString(11)%>" readonly></td>
				<td><input type="text" name="principalpayable<%=cnt%>" size="8" value="<%=rs.getString(12)%>" readonly></td>
				<td><input type="text" name="totalaccruedprincipal<%=cnt%>" size="8" value="<%=rs.getString(13)%>" readonly></td>
...
<%
	...
	{
		out.println("QDFIR 541 View Error: "+e.toString());
	}
		cn.close();
%>
  </table>
